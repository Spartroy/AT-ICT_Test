  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-300 text-sm">Manage Hall of Fame students and student stories from one place.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-gray-900/60 border border-gray-700 rounded-xl p-5">
          ...
          <form onSubmit={submitHallEntry} className="space-y-3 mb-6">
            <input className="w-full bg-gray-800 border border-gray-600 rounded-xl px-3 py-2 text-white" placeholder="Student name" value={hallForm.name} onChange={(e) => setHallForm((prev) => ({ ...prev, name: e.target.value }))} required />
            <input className="w-full bg-gray-800 border border-gray-600 rounded-xl px-3 py-2 text-white" placeholder="Year (e.g. 2026)" value={hallForm.year} onChange={(e) => setHallForm((prev) => ({ ...prev, year: e.target.value }))} required />
            <div className="flex gap-2">
              ...
              {editingHallId && <button type="button" onClick={() => { setEditingHallId(null); setHallForm(emptyHallForm); }} className="border border-gray-600 text-gray-300 px-4 py-2 rounded-xl">Cancel</button>}
            </div>
          </form>
          <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
            {!loading && hallEntries.map((entry) => (
              <div key={entry._id} className="flex items-center justify-between bg-gray-800/60 rounded-xl p-3">
                <div>
        ...
        <div className="bg-gray-900/60 border border-gray-700 rounded-xl p-5">
          ...
          <form onSubmit={submitStory} className="space-y-3 mb-6">
            <input className="w-full bg-gray-800 border border-gray-600 rounded-xl px-3 py-2 text-white" placeholder="Student name" value={storyForm.name} onChange={(e) => setStoryForm((prev) => ({ ...prev, name: e.target.value }))} required />
            <input className="w-full bg-gray-800 border border-gray-600 rounded-xl px-3 py-2 text-white" placeholder="Country" value={storyForm.country} onChange={(e) => setStoryForm((prev) => ({ ...prev, country: e.target.value }))} required />
            <textarea className="w-full bg-gray-800 border border-gray-600 rounded-xl px-3 py-2 text-white min-h-[110px]" placeholder="Story text" value={storyForm.text} onChange={(e) => setStoryForm((prev) => ({ ...prev, text: e.target.value }))} required />
            <div className="flex gap-2">
              <button type="submit" className="bg-[#CA133E] hover:bg-[#A01030] text-white px-4 py-2 rounded-xl">{editingStoryId ? 'Update' : 'Add'}</button>
              {editingStoryId && <button type="button" onClick={() => { setEditingStoryId(null); setStoryForm(emptyStoryForm); }} className="border border-gray-600 text-gray-300 px-4 py-2 rounded-xl">Cancel</button>}
            </div>
          </form>
          <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
            {!loading && stories.map((story) => (
              <div key={story._id} className="bg-gray-800/60 rounded-xl p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-white font-semibold">{story.name}</p>
                    <p className="text-gray-400 text-xs">{story.country}</p>
                    <p className="text-gray-300 text-sm mt-1 line-clamp-2">{story.text}</p>
                  </div>
                  <div className="flex gap-2">
