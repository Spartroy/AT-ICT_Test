...
const NotesManagement = () => {
  ...
  const NoteCard = ({ note }) => (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#161616] rounded-xl border border-white/5 p-4 hover:bg-white/8">
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 bg-[#161616] border border-white/5 rounded-xl p-4 sm:p-6">
        <div className="flex-1">
          <h2 className="text-lg sm:text-xl lg:text-xl font-bold text-white">Interactive Notes</h2>
          <p className="text-sm text-gray-400 mt-1">Add Prezi notes by Chapter and Phase</p>
        </div>
        <motion.button onClick={() => setShowCreateModal(true)} className="flex items-center space-x-2 bg-[#CA133E] hover:bg-[#A01030] text-white px-4 py-2 rounded-xl" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
      ...
      <div className="bg-[#161616] border border-white/5 rounded-xl p-4 sm:p-6">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <input type="text" placeholder="Search notes..." value={filters.search} onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))} className="w-full px-3 py-2 bg-white/5 border border-white/5 rounded-xl text-white placeholder-gray-400" />
          </div>
          <button onClick={() => setShowFilters(!showFilters)} className="ml-3 flex items-center space-x-2 px-4 py-2 bg-white/5 text-gray-400 rounded-xl hover:bg-gray-600/50"><FunnelIcon className="h-5 w-5" />{showFilters ? <ChevronUpIcon className="h-4 w-4" /> : <ChevronDownIcon className="h-4 w-4" />}</button>
        </div>
        <AnimatePresence>
          {showFilters && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <select value={filters.phase} onChange={(e) => setFilters(prev => ({ ...prev, phase: e.target.value }))} className="px-3 py-2 bg-white/5 border border-white/5 rounded-xl text-white">
      ...
      {pagination.pages > 1 && (
        <div className="flex justify-center items-center space-x-2">
          <button onClick={() => setPagination(prev => ({ ...prev, current: prev.current - 1 }))} disabled={!pagination.hasPrev} className="px-4 py-2 bg-white/5 text-gray-400 rounded-xl disabled:opacity-50">Previous</button>
          <span className="text-gray-400">Page {pagination.current} of {pagination.pages}</span>
          <button onClick={() => setPagination(prev => ({ ...prev, current: prev.current + 1 }))} disabled={!pagination.hasNext} className="px-4 py-2 bg-white/5 text-gray-400 rounded-xl disabled:opacity-50">Next</button>
        </div>
      )}
      ...
      <AnimatePresence>
        {(showCreateModal || showEditModal) && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#1A1A1A] rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} className="w-full px-3 py-2 bg-white/5 border border-white/5 rounded-xl text-white" placeholder="Enter note title" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-400 mb-2">Phase</label>
                    <select value={formData.phase} onChange={(e) => setFormData(prev => ({ ...prev, phase: e.target.value }))} className="w-full px-3 py-2 bg-white/5 border border-white/5 rounded-xl text-white">
                      <option value="1">Phase 1</option>
                      <option value="2">Phase 2</option>
                      <option value="3">Phase 3</option>
                    </select>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">Prezi Link</label>
                  <input type="url" value={formData.linkUrl} onChange={(e) => setFormData(prev => ({ ...prev, linkUrl: e.target.value }))} className="w-full px-3 py-2 bg-white/5 border border-white/5 rounded-xl text-white" placeholder="https://prezi.com/..." />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-400 mb-2">Order (optional)</label>
                  <input type="number" value={formData.order} onChange={(e) => setFormData(prev => ({ ...prev, order: e.target.value }))} className="w-full px-3 py-2 bg-white/5 border border-white/5 rounded-xl text-white" placeholder="0" />
                </div>
              </div>
      ...
      <AnimatePresence>
        {showDeleteModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#1A1A1A] rounded-xl p-6 w-full max-w-md">
